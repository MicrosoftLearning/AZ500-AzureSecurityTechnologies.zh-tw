...
2. 在 [虛擬機器] **刀鋒視窗上，按一下 [+ 建立 **]，然後**在**下拉式清單中按一下 [虛擬機器**]。**
...
6. 在 [建立虛擬機器 **] 刀鋒視窗的 [網路] **** 索引標籤上，選取先前建立的網路 **myVirtualNetwork** 和**預設 （10.0.0.0/24）** 子**網路。
...
1. 在 Azure 入口網站中，流覽回 [虛擬機器] **刀鋒視窗，按一下 **[+ 建立]**，然後在下拉式清單中按一下 **[虛擬機器**]。**
...
5. 在 [建立虛擬機器 **] 刀鋒視窗的 [網路] **** 索引標籤上，選取先前建立的網路 **myVirtualNetwork** 和**預設 （10.0.0.0/24）** 子**網路。
...
#### 作業 3：將每個虛擬機器的網路介面與其應用程式安全性群組建立關聯。
...
7. 按一下 + **新增應用程式安全性群組**，在 [應用程式安全性群組 **] **清單中，選取 [**myAsgMgmtServers**]，然後按一下 **[新增**]。
...
1. 按一下 Azure 入口網站右上方的第一個圖示，開啟 Cloud Shell。 如果出現提示，請選取 **[PowerShell** ] 和 **[不需要**儲存體帳戶]，選取訂用帳戶的名稱，然後選取 [ **套用**]。
