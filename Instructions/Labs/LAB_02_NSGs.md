...
- 工作 1：建立具有一個子網的虛擬網路。
- 工作2：建立兩個應用程式安全組。
- 工作 3：建立網路安全組，並將其與虛擬網路子網產生關聯。
- 工作 4：建立連入 NSG 安全性規則，將所有流量傳送至 Web 伺服器，以及 RDP 到管理伺服器。
...
4. 在 [**建立虛擬網络 **] 刀鋒視窗的 **[基本]** 索引標籤上，指定下列設定（讓其他人保留預設值），然後按 **[下一步：IP 位址**]：
...
> 結果：您已部署虛擬網路、具有輸入安全性規則的網路安全性，以及兩個應用程式安全組。 
...
3. 在 [建立虛擬機器] 刀鋒視窗的 [基本] 索引標籤上，指定下列設定 (將其他設定保留為預設值)：
   ...
   |大小|**標準 D2s v3**|
   |使用者名稱|**Student**|
   |密碼|**請建立您自己的密碼，並將其記錄在後續實驗室中以供日後參考**|
   |確認密碼|**重新輸入您的密碼**|
   |公用輸入連接埠|**None**|
   |您要使用現有的 Windows Server 授權嗎 |**否**|

    >**注意**：針對公用輸入埠，我們將依賴預先建立的NSG。 

4. 按兩下 **[下一步：磁碟>**，**然後在 [建立虛擬機 **] 刀鋒視窗的 **[磁碟**] 索引卷標上，將**OS磁碟類型**設定為**標準 HDD**，然後按 **[下一步：網路>**。

5. 在 [建立虛擬機器]**** 刀鋒視窗的 [網路]**** 索引標籤上，選取先前建立的網路 **myVirtualNetwork**。

6. 在 [NIC 網路安全性群組]**** 下方選取 [無]****。

7. 按 **[下一步：管理>**]，然後按 **[下一步：監視>**。 在 [**建立虛擬機**] 刀鋒視窗的 [**監視]** 索引標籤上，確認下列設定：
...
3. 按兩下 **[下一步：磁碟>**，**然後在 [建立虛擬機 **] 刀鋒視窗的 **[磁碟**] 索引卷標上，將**OS磁碟類型**設定為**標準 HDD**，然後按 **[下一步：網路>**。

4. 在 [建立虛擬機器]**** 刀鋒視窗的 [網路]**** 索引標籤上，選取先前建立的網路 **myVirtualNetwork**。

5. 在 [NIC 網路安全性群組]**** 下方選取 [無]****。

6. 按 **[下一步：管理>**]，然後按 **[下一步：監視>**。 在 [**建立虛擬機**] 刀鋒視窗的 [**監視]** 索引標籤上，確認下列設定：
...
7. 在 [檢閱 + 建立]**** 刀鋒視窗中按一下 [檢閱 + 建立]****，確定驗證成功後再點選 [建立]****。
    ...
    >**注意**：請等候這兩部虛擬機布建，再繼續進行。 

#### 工作3：將每個虛擬機網路介面與其應用程式安全組產生關聯。
...
3. 在 myVMWeb** 刀鋒視窗的 [** 網络 **] 區段中，** 按兩下 [**網络設定**]，然後在 [myVMWeb \| 網络設定 **] 刀鋒視窗上**，按兩下 **[應用程式安全組] 索引標籤**。

4. 按兩下 [+ **新增應用程式安全組**]，在 [ **應用程式安全組** ] 清單中，選取 **[myAsgWebServers**]，然後按兩下 [ **儲存**]。

5. 返回 [虛擬機器]**** 刀鋒視窗，然後在虛擬機器清單中，按一下 **myVMMgmt** 項目。

6. 在 [myVMMgmt **] 刀鋒視窗**的 [** 網络**] 區段中，按兩下 **[網络設定]，然後在 [myVMMgmt \| 網络設定****] 刀鋒視窗上**，按兩下 **[應用程式安全組] 索引標籤**。

7. 按兩下 [+ **新增應用程式安全組**]，在 [ **應用程式安全組** ] 清單中，選取 **[myAsgMgmtServers**]，然後按兩下 [ **儲存**]。
...
3. 按一下 [下載 RDP 檔案]****，然後使用該檔案透過遠端桌面連線至 **myVMMgmt** Azure 虛擬機器。 出現驗證提示時，請提供下列認證：
   ...
   |密碼|**請使用實驗室 02 >練習 1 >工作 1 >步驟 9 中建立的個人密碼。**|
    ...
    >**注意**：確認遠端桌面連線成功。 此時您已確認可以透過遠端桌面連線至 myVMMgmt。

4. 在 Azure 入口網站中前往 [myVMWeb]**** 虛擬機器刀鋒視窗。

5. 在 myVMWeb** 刀鋒視窗的 [** 承載 **] 區段中，** 按兩下 **[執行] 命令**，然後按兩下 [**RunPowerShellScript**]。
...
6. 在 [執行命令指令碼]**** 窗格中執行下列命令，以便在 **myVmWeb** 上安裝網頁伺服器角色：
    ...
    >**注意**：等候安裝完成。 這可能需要幾分鐘的時間。 此時您可以驗證是否能透過 HTTP/HTTPS 存取 myVMWeb。
...
9. 開啟另一個瀏覽器分頁，前往您在上一個步驟找到的 IP 位址。
    ...
    >**注意**：瀏覽器頁面應該會顯示預設的 IIS 歡迎頁面，因為根據 myAsgWebServers** 應用程式安全組的**設定，允許從因特網輸入埠 80。 myVMWeb Azure 虛擬機器的網路介面已與該應用程式安全性群組建立關聯。 

> 結果：您已驗證 NSG 和 ASG 組態是否正常運作，且流量正在正確管理。 
