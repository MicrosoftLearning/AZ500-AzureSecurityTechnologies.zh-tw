...
2. 在 [ **使用者 \| 所有使用者]** 刀鋒視窗中，按一下 **[+ 新增使用者** ]，然後按一下 [ **建立新使用者**]。 

3. 在 [**新增使用者**] 刀鋒視窗上，確定已選取 [**建立使用者**] 選項，並在 [基本] 索引標籤上指定下列設定， (讓其他人保持預設值) ，然後按 [**下一步：屬性 >：**
   ...
   |密碼|確定已選取 [ **自動產生密碼]** 選項|
      ...
      >**注意**：記錄完整的使用者主體名稱。 您可以按一下下拉式清單右側的 [ **複製到剪貼簿** ] 按鈕來複製其值，以顯示功能變數名稱。 之後在此實驗室中會用到。
   
      >**注意**：記錄使用者密碼。 按一下文字方塊右側的 [ **複製到剪貼簿** ] 按鈕，即可複製其值。 之後在此實驗室中會用到。 
   
4. 在 [**屬性] 索引**標籤上，捲動至底部並指定 [使用位置]：**美國** (讓其他人保持預設值) ，然後按 [**下一步：指派 >**。

5. 在 [ **指派] 索引** 標籤上，按一下 **[+ 新增角色** ]，然後搜尋並選取 [ **全域管理員**]。 按一下 **[選取** ] 和 [ **檢閱 + 建立** ]，然後按一下 [ **建立**]。

6. 返回 [使用者 \| 所有使用者] 刀鋒視窗，按一下 [+ 新增使用者]。 

7. 在 [ **新增使用者** ] 刀鋒視窗中，確定已選取 [ **建立使用者** ] 選項，並指定下列設定， (讓其他人保持預設值) ，然後按 [ **下一步：屬性] >**：
...
8. 在 [**屬性] 索引**標籤上，捲動至底部並指定 [使用位置]：**美國** (讓其他人保持預設值) ，然後按一下 [**檢閱 + 建立****]，** 然後按一下 [建立]。

9. 返回 [使用者 \| 所有使用者] 刀鋒視窗，按一下 [+ 新增使用者]。 

10. 在 [ **新增使用者** ] 刀鋒視窗中，確定已選取 [ **建立使用者** ] 選項，並指定下列設定， (讓其他人保持預設值) ，然後按 [ **下一步：屬性] >**：
...
11. 在 [**屬性] 索引**標籤上，捲動至底部並指定 [使用位置]：**美國** (讓其他人保持預設值) ，然後按一下 [**檢閱 + 建立****]，** 然後按一下 [建立]。
...
2. 在顯示使用者帳戶屬性的刀鋒視窗上，按一下 [編輯屬性]。  確認 [使用位置] 設定為 **[美國**]。 如果沒有，請設定使用位置，然後按一下 [ **儲存**]。
...
7. 回到 [ **指派授權]** 刀鋒視窗，按一下 [ **指派選項**]，確定已啟用所有選項，按一下 [ **檢閱 + 指派**]，然後按一下 [ **指派**]。
...
6. 切換至 **[使用者** ] 索引標籤，按一下 **aaduser1** 專案、按一下 [ **啟用** ] 連結，然後在出現提示時，按一下 **[啟用多重要素驗證** ]，然後按一下 [ **關閉**]。
...
14. 流覽回 **AdatumLab500-04** Azure Active Directory 租使用者刀鋒視窗，在 [ **管理** ] 區段中，按一下 [ **內容**]，下一步按一下刀鋒視窗底部的 [ **管理安全性預設值** ] 連結，在 [ **啟用安全性預設值** ] 刀鋒視窗上，按一下 [ **已停用**]。 選取 **[我的組織使用條件式存取** ] 作為 *停用的原因*，按一下 [ **儲存**]，閱讀警告，然後按一下 [ **停用**]。
...
2. 流覽至 Azure 入口網站 ， **`https://portal.azure.com/`** 並使用**aaduser1**使用者帳戶登入。 
...
8. 如果系統提示您輸入其他驗證方法，請按一下 **[我想要使用不同的方法**]，從下拉式清單中選取 **[Email**]，按一下 **[確認**]，提供您想要使用的電子郵件地址，然後按 [**下一步**]。 收到對應的電子郵件後，在電子郵件本文中找出並提供驗證碼，接著按一下 [完成]。
...
5. 在 [新增] 刀鋒視窗中，指定下列設定：
   ...
   - 在 [ **使用者]** 底下，按一下 **[0 選取的使用者和群組**]。 在右側的 [包含]、[啟用 **選取使用者和群組** ] >> 選取 [ **使用者和群組** ] 核取方塊，在 [ **選取使用者和群組** ] 刀鋒視窗上，選取 **aaduser2** 核取方塊，然後按一下 [ **選取**]。
    
   - 在 **[目標資源]** 底下，按一下 **[未選取目標資源**]，按一下 [ **選取應用程式**] 底下的 [ **無**]。 在 [ **選取** ] 刀鋒視窗上，選取 **Microsoft Azure 管理的** 核取方塊，然後按一下 [ **選取**]。 

     >**注意**：檢閱此原則會影響 Azure 入口網站存取權的警告。
    
   - 在 [ **條件**] 底下，按一下 **[選取 0 個條件**]，在 [ **登入風險** ] 底下，按一下 [ **未設定**]。 在 [ **登入風險]** 刀鋒視窗上，檢閱風險層級，但不進行任何變更並關閉 [ **登入風險** ] 刀鋒視窗。
    
   - 在 **[裝置平臺**] 底下，按一下 [ **未設定**]，檢閱可包含但未進行任何變更的裝置平臺，然後按一下 [ **完成**]。
    
   - 在 **[位置]** 底下，按一下 [ **未設定** ] 並檢閱位置選項，而不進行任何變更。
    
   - 在 [**存取控制**] 區段的 [**授**與] 下，按一下**已選取的 0 個控制項**。 在 [ **授與]** 刀鋒視窗上，選取 **[需要多重要素驗證** ] 核取方塊，然後按一下 [ **選取]**
...
4. 在 **[指派****使用者**] 底下  >  ，按一下 **[0 選取的使用者和群組**]。

5. 在 **[包含**] 底下，按一下 **[選取使用者和群組**]、[**使用者和群組**]、選取**aaduser2 和 aaduser3**，然後按一下 [**選取**]。** **

6. 在 **[排除]** 下按一下 [ **使用者和群組**]，選取 **aaduser1** ，然後按一下 [ **選取**]。 

7. 在 **[目標資源**] 底下，按一下 **[未選取目標資源**]，確認下拉式清單中已選取 [ **雲端應用程式** ]，然後在 [ **包含**] 底下選取 [ **所有雲端應用程式**]。

8. 在 [ **條件**] 底下，按一下 **[選取 0 個條件**]，在 [ **使用者風險** ] 底下，按一下 [ **未設定**]。 在 [ **使用者風險** ] 刀鋒視窗上，將 [ **設定** ] 設定為 [ **是**]。

9. 在 [設定原則強制執行所需的使用者風險等級] 下，選取 [高]。

10. 按一下 [完成]。

11. 在 [**存取控制**] 區段中的 [**授**與] 下，按一下**已選取的 0 個控制項**。 在 [ **授與]** 刀鋒視窗上，確認已選取 [ **授與存取權** ]。

12. 選取 [需要多重要素驗證] **和 [需要變更密碼]** 。

13. 按一下 [選取]。

14. 在 **[會話**] 下，按一下 **已選取的 0 個控制項**。 選取 **[登入頻率**]，然後選取 **[每次]。**
...
4. 在 **[指派使用者]**  >  底下，按一下 **[0 選取的使用者和群組**]。** **

5. 在 **[包含**] 底下，依序按一下 [ **選取使用者和群組**]、[ **使用者和群組**]、選取 **aaduser2** 和 **aaduser3** ，然後按一下 [ **選取**]。

6. 在 **[排除]** 下，按一下 [ **使用者和群組** ] 選取 **aaduser1** ，然後按一下 [ **選取**]。 

7. 在 **[目標資源**] 底下，按一下 **[未選取目標資源**]，確認下拉式清單中已選取 [ **雲端應用程式** ]，然後在 [ **包含**] 底下選取 [ **所有雲端應用程式**]。

8. 在 [ **條件]** 底下，按一下 **[選取 0 個條件**]，在 [ **登入風險** ] 底下，按一下 [ **未設定**]。 在 [ **登入風險** ] 刀鋒視窗上，將 [ **設定** ] 設定為 [ **是**]。
...
1. 在Azure 入口網站中，將 **[目錄 + 訂**用帳戶] 篩選器設定為您部署**az500-04-vm1** Azure VM 的 Azure 訂用帳戶相關聯的 Azure AD 租使用者。
...
11. 安裝完成後，請啟動 ToR 瀏覽器、使用初始頁面上的 **[連線**] 選項，然後流覽至位於 **https://myapps.microsoft.com** 的應用程式存取面板。

12. 出現提示時，嘗試使用 **aaduser3** 帳戶登入。 

    >**注意**：您會看見「您的登入已遭封鎖」訊息。 這是預期情況，因為此帳戶未設定多重要素驗證，因為與使用 ToR 瀏覽器相關聯的登入風險增加而需要此帳戶。

13. 在 ToR 瀏覽器中，選取返回箭號，以您稍早在此實驗室中建立並設定多重要素驗證的 **aaduser1** 帳戶登入。

    >**注意**：這次您會看見「偵測到可疑的活動」訊息。 同樣地，這是預期的，因為此帳戶已設定多重要素驗證。 由於使用 ToR 瀏覽器時登入風險會增加，因此您必須使用多重要素驗證。
