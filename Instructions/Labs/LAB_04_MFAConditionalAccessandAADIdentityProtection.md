...
2. 在顯示使用者帳戶屬性的刀鋒視窗上，按一下 [編輯屬性]。  如果尚未設定使用位置，請將 [使用位置] 設定為[美國]，然後按一下 [儲存]。
...
7. 返回 [指派授權] 刀鋒視窗，按一下 [指派選項]，確定已啟用所有選項，再依序按一下 [檢閱 + 指派] 與 [指派]。
...
2. 瀏覽至 **AdatumLab500-04** [Azure AD 租用戶] > [安全性] > [條件式存取]。

3. 按一下 [新增原則]。

4. 在 [名稱] 文字方塊中，鍵入原則名稱 **AZ500Policy2**。

5. 在 [指派] 下，選取 [使用者]。

6. 在 [包含] 下方，按一下 [選取使用者和群組]，再選取 **aaduser2** 和 **aaduser3**。

7. 在 [排除] 下方，按一下 [使用者和群組]，選取 **aaduser1**。 

8. 在 [雲端應用程式或動作]  >  [包括] 底下，選取 [所有雲端應用程式]。

9. 在 [條件]  >  [使用者風險] 底下，將 [設定] 設為 [是]。

10. 在 [設定原則強制執行所需的使用者風險等級] 下，選取 [高]。

11. 按一下 [完成]。

12. 在 [存取控制] 下方，確認已啟用 [授與]。    

13. 選取 [需要多重要素驗證] **和 [需要變更密碼]** 。

14. 按一下 [選取]。

15. 在 [工作階段] 下方，按一下 [登入頻率]，並確認已啟用 [每次]。

16. 按一下 [選取]。

17. 確認您的設定，並將 [啟用原則] 設為 [報告專用]。

    >**注意**：頁面底部會顯示下列訊息： **「您似乎想要管理您組織的安全性設定。真棒! 您必須先停用安全性預設值，才能啟用條件式存取原則。」**

18. 按一下訊息中的**停用安全性預設值**部分。

19. 將 [啟用安全性預設值] 從 [是] 變更為 [否]。

20. 按一下 [其他]，然後在欄位中鍵入下列內容：**AZ500 lab use**。
...
5. 在 [包含] 下方，按一下 [選取使用者和群組]，再選取 **aaduser2** 和 **aaduser3**。

6. 在 [排除] 下方，按一下 [使用者和群組]，選取 **aaduser1**。 

7. 在 [雲端應用程式或動作]  >  [包括] 底下，選取 [所有雲端應用程式]。

8. 在 [條件]  >  [登入風險] 底下，將 [設定] 設為 [是]。
...
12. 選取 [授與存取權]、[需要多重要素驗證]。

13. 按一下 [選取]。

13. 在 [工作階段] 下方，選取 [登入頻率]，並確認已啟用 [每次]。

14. 按一下 [選取]。

15. 確認您的設定，並將 [啟用原則] 設為 [報告專用]。
