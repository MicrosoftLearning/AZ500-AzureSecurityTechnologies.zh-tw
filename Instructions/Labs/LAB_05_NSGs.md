---
lab:
  title: 05 - 網路安全性群組和應用程式安全性群組
  module: Module 02 - Implement Platform Protection
---

# 實驗室 05：網路安全性群組和應用程式安全性群組
# 學生實驗室手冊
...
- 工作 1：建立具有一個子網的虛擬網路。
- 工作 2：建立兩個應用程式安全性群組。
- 工作 3：建立網路安全性群組，並將其與虛擬網路子網產生關聯。
- 工作 4：建立連入 NSG 安全性規則，將所有流量傳送至 Web 服務器，以及 RDP 到管理伺服器。
...
- 工作 1：建立要作為網頁伺服器的虛擬機器。
- 工作 2：建立虛擬機器以做為管理伺服器。 
- 工作 3：將每個虛擬機器網路介面與其應用程式安全性群組建立關聯。
- 工作 4：測試網路流量篩選。
...
7. 按 ** [下一步：管理> ** ]，然後按 ** [下一步：監視> ** 。 在 [ ** 建立虛擬機器 ** ] 刀鋒視窗的 [ ** 監視] ** 索引標籤上，確認下列設定：
...
3. 按一下 ** [下一步：磁片> ** ， ** 然後在 [建立虛擬機器 ** ] 刀鋒視窗的 ** [磁片 ** ] 索引標籤上，將 ** OS 磁片類型 ** 設定為 ** 標準 HDD ** ，然後按 ** [下一步：網路> ** 。
...
7. 在 [檢閱 + 建立]**** 刀鋒視窗中按一下 [檢閱 + 建立]****，確定驗證成功後再點選 [建立]****。
    ...
    >**注意 ** ：請等候這兩部虛擬機器布建，再繼續進行。 

#### 工作 3：將每個虛擬機器網路介面與其應用程式安全性群組產生關聯。
...
3. 在 myVMWeb ** 刀鋒視窗的 [ ** 網路 ** ] 區段中， ** 按一下 [ ** 網路設定 ** ]，然後在 [myVMWeb \| 網路設定 ** ] 刀鋒視窗上 ** ，按一下 ** [應用程式安全性群組] 索引卷 ** 標。

4. 按一下 [+ ** 新增應用程式安全性群組 ** ]，在 [ ** 應用程式安全性群組 ** ] 清單中，選取 ** [myAsgWebServers ** ]，然後按一下 [ ** 儲存 ** ]。

5. 返回 [虛擬機器]**** 刀鋒視窗，然後在虛擬機器清單中，按一下 **myVMMgmt** 項目。

6. 在 [myVMMgmt ** ] 刀鋒視窗 ** 的 [ ** 網路 ** ] 區段中，按一下 ** [網路設定]，然後在 [myVMMgmt \| 網路設定 ** ** ] 刀鋒視窗上 ** ，按一下 ** [應用程式安全性群組] 索引卷 ** 標。

7. 按一下 [+ ** 新增應用程式安全性群組 ** ]，在 [ ** 應用程式安全性群組 ** ] 清單中，選取 ** [myAsgMgmtServers ** ]，然後按一下 [ ** 儲存 ** ]。
...
5. 在 myVMWeb ** 刀鋒視窗的 [ ** 承載 ** ] 區段中， ** 按一下 ** [執行] 命令 ** ，然後按一下 [ ** RunPowerShellScript ** ]。
...
9. 開啟另一個瀏覽器分頁，前往您在上一個步驟找到的 IP 位址。
    ...
    >**注意 ** ：瀏覽器頁面應該會顯示預設的 IIS 歡迎頁面，因為根據 myAsgWebServers ** 應用程式安全性群組的 ** 設定，允許從網際網路輸入埠 80。 myVMWeb Azure 虛擬機器的網路介面已與該應用程式安全性群組建立關聯。 
