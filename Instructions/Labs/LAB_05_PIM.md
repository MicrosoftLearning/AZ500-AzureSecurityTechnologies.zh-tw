...
1. 登入位於 的 **`https://portal.azure.com/`** Azure 入口網站。
...
4. 在 [ **全域讀取器 \| 指派** ] 刀鋒視窗上，按一下刀鋒視窗工具列中的 **[角色設定** ] 圖示，然後檢閱角色的組態設定，包括 Azure Multi-Factor Authentication 需求。
...
8. 在左側導覽面板中，按一下 [ **指派**]。 在 [**合格指派] 索引**標籤的 [**安全性系統管理員**] 下，針對**aaduser2**指派選取 **[更新**]。 依序選取 [永久合格] 與 [儲存]。
...
9. 返回 [我的角色 \| Azure AD 角色] 刀鋒視窗，按一下 [使用中指派] 索引標籤。請注意 [使用中指派] 內包含**安全性系統管理員**，但不含**全域讀取者**角色。
...
14. 在 [核准要求 \| Azure AD 角色] 刀鋒視窗的 [要求啟用角色] 區段，勾選代表 **aaduser2** 所提出「全域讀取者」角色啟用要求的項目核取方塊。 
...
17. 在 InPrivate 瀏覽器中，以**aaduser2**身分登入 at **`https://portal.azure.com/`** Azure 入口網站
...
20. 在 [我的角色 \| Azure AD 角色] 刀鋒視窗中，切換至 [使用中指派] 索引標籤，並確認全域讀取者角色已啟用。
