...
3. 在 [ **新增使用者** ] 刀鋒視窗中，確定已選取 [ **建立使用者** ] 選項，在 [基本] 索引標籤上指定下列設定， (讓其他人保持預設值) ，然後按 [ **下一步]：[屬性] >**：
   ...
   |密碼|確定已選取 [自動產生密碼] 選項，然後按一下 [顯示密碼]|
    ...
    >**注意**：記錄完整使用者名稱。 您可以按一下下拉式清單右側的 [ **複製到剪貼簿** ] 按鈕來複製其值，以顯示功能變數名稱，並將它貼到記事本檔中。 之後在此實驗室中會用到。

    >**注意**：按一下 [密碼] 文字方塊右側的 [ **複製到剪** 貼簿] 按鈕，然後將它貼到記事本檔中，以記錄使用者的密碼。 之後在此實驗室中會用到。

4. 在 [**屬性] 索引**標籤上，捲動至底部並指定 [使用位置]：**美國** (讓其他人保持預設值) ，然後按 [**下一步：指派 >**。

5. 在 [ **指派] 索引** 標籤上，按一下 **[+ 新增角色**]，搜尋並選取 **[全域管理員**]，然後按一下 [ **選取**]。 按一下 [檢閱 + 建立]  ，然後按一下 [建立]  。
   
    >**注意**：唯有具備全域管理員角色的 Azure AD 使用者才能實作 Azure AD Connect。

6. 開啟 InPrivate 瀏覽器視窗。

7. 流覽至 Azure 入口網站， **`https://portal.azure.com/`** 並使用**syncadmin**使用者帳戶登入。 出現提示時，請將您稍早在此工作中記錄的密碼變更為符合複雜度需求的密碼，並記錄以供日後參考。 在稍後的工作中，系統會提示您輸入此密碼。
...
5. 在 [ **IP 位址** ] 下拉式清單中，選取 **[負載平衡器公用 IP 位址**]，然後按一下 [ **下載 RDP 檔案** ]，並用它來透過遠端桌面連線到 **adVM** Azure VM。 出現驗證提示時，請提供下列認證：
   ...
   |密碼|**請使用您在實驗室 04 > 練習 1 > 工作 1 > 步驟 9 中建立的個人密碼。**|
    ...
    >**注意**：如果 [RDP] 刀鋒視窗的 [**IP 位址**] 下拉式清單中無法使用**負載平衡器公用 IP 位址**，請在 Azure 入口網站中搜尋**公用 IP 位址**，選取 **[adPublicIP**]，並記下其 IP 位址。 按一下 [開始] 按鈕，輸入 **MSTSC** ，然後按 **Enter** 以啟動遠端桌面用戶端。 在 [ **電腦：]** 文字方塊中輸入負載平衡器的公用 IP 位址，然後按一下 [ **連線**]。
...
9. 按兩下新建立的 **ToSync** 組織單位，使其內容會出現在 Active Directory 管理中心主控台的詳細資料窗格中。 
...
3. 在 [Azure 入口網站] 的 [**AdatumSync 概 \| 觀**] 刀鋒視窗中，按一下 [**管理**] 下方左側導覽面板中的 **[Azure AD Connect**]。

4. 在 **[AAD Connect \| 開始使用]** 刀鋒視窗上，按一下左側導覽面板中的 [ **連線同步]** ，然後按一下 [ **下載 Azure AD Connect** ] 連結。 系統會將您重新導向至 **Azure AD Connect** 下載頁面。

5. 在 **[Azure AD Connect** 下載] 頁面上，按一下 [ **下載**]。
...
16. 在 [ **網域和 OU 篩選** ] 頁面上，按一下 [ **同步選取的網域和 OU** ] 選項，然後清除功能變數名稱旁 **的核取方塊 adatum.com**。 按一下以展開 **adatum.com**，只選取 **ToSync** OU 旁的核取方塊，然後按 [ **下一步**]。
...
3. 按一下 **aduser1** 帳戶，然後選取 [ **屬性] 索引** 標籤。向下捲動至 [ **內部部署** ] 區段，請注意， **已啟用內部部署同步** 處理的屬性設定為 **[是**]。

4. 在 **aduser1** 刀鋒視窗的 [ **作業資訊** ] 區段中，請注意未設定 **Department** 屬性。

5. 在**adVM**的遠端桌面會話中，切換至**Active Directory 管理中心**，在**ToSync** OU 的物件清單中選取**aduser1**專案，然後在 [**工作**] 窗格的 **[aduser1**] 區段中，選取 [**屬性**]。
...
9. 切換至顯示 **aduser1** 刀鋒視窗的 Microsoft Edge 視窗，重新整理頁面，並注意 Department 屬性設定為 [銷售]。
    ...
    >**注意**：如果 **未設定 Department** 屬性，您可能需要等候最多三分鐘，然後重新整理頁面。
