...
#### 安裝az500-10-DB.json自定義範本

1. 在 Azure 入口網站頁面頂端的 [搜尋資源、服務及文件] 文字輸入框中輸入****「部署自訂範本」****，然後按下 **Enter** 鍵。

2. 在 [自訂部署****] 刀鋒視窗中按一下 ****[在編輯器中建置您自己的範本] 選項。

3. 在 [編輯範本]** 刀鋒視窗上，按兩下 **[** 載入檔案**]，找出 **\Allfiles\Labs\10\az-500-10_DB.json** 檔案，然後按兩下 [**開啟**]。

4. 確定已選取正確的資源群組。

5. 將 [ **管理員密碼** ] 設定為您用於上一個步驟的相同密碼。
...
4. 流覽回 **az500-10-vm1** 刀鋒視窗，單擊 **[概觀**]，下一步單擊 **[連線** ]，然後在下拉菜單中，單擊 [ **連線**]。 

5. 下載 RDP 檔案，並使用它透過遠端桌面連線到 **az500-10-vm1** Azure VM。 出現驗證提示時，請提供下列認證：
...
- 工作 1：安裝 Visutal Studio 2022
- 工作 2：執行資料驅動應用程式，以示範如何使用 Azure 金鑰保存庫 來加密 Azure SQL 資料庫

#### 工作 1：安裝 Visual Studio 2022

1. 如果您還沒有到伺服器虛擬機，請切換至您的伺服器虛擬機。

2. 開啟 [伺服器管理員] 。

3. 選取 [本機伺服器]。

4. 將 [IE 增強式安全性設定 **] 設定**為 **[關閉**]。

5. 開啟瀏覽器並略過關閉 IE ESC 的相關警告。

6. 移至 https://visualstudio.microsoft.com/downloads。

7. 在 [**Visual Studio 2022**] 方塊中，於 [社群 **] 下**選取 [**免費下載**]。
