...
![顯示本實驗室中任務流程的圖表。](../media/key-vault-diagram.png)
...
#### 安裝az500-10-DB.json自訂範本

1. 在 Azure 入口網站頁面頂端的 [搜尋資源、服務及文件] 文字輸入框中輸入****「部署自訂範本」****，然後按下 **Enter** 鍵。

2. 在 [自訂部署****] 刀鋒視窗中按一下 ****[在編輯器中建置您自己的範本] 選項。

3. 在 [編輯範本] **刀鋒視窗上，按一下 **[載入檔案**]，找出 **\Allfiles\Labs\10\az-500-10_DB.json** 檔案，然後按一下 **[開啟****]。

4. 請確定已選取正確的資源群組。

5. 將管理員密碼**設定**為您用於上一個步驟的相同密碼。
...
9. 在 [建立存取原則]**** 刀鋒視窗上，指定下列設定 (將所有其他設定維持為預設值)： 
    ...
    |選取主體|在 [主體] **** 刀鋒視窗上，選取您的使用者帳戶，然後按一下 **[下一步]**|
    |應用程式 (選擇性)|按一下 [下一步]****|
    |檢閱 + 建立|按一下 [建立]****|
...
4. 流覽回 **az500-10-vm1** 刀鋒視窗，按一下 **[概觀**]，然後按一下 [連線 **]，然後在**下拉式功能表中按一下 **[連線**]。 
...
1. 如果您還沒有伺服器虛擬機器，請切換至您的伺服器虛擬機器。

2. 開啟 [伺服器管理員]。

3. 選取 [本機伺服器]。

4. 將 IE 增強安全性設定設定為****關閉**。**

5. 打開瀏覽器並繞過有關 IE ESC 關閉的警告。

6. 移至 https://visualstudio.microsoft.com/downloads。

7. 在 [Visual Studio 2022] **方塊的 [社群]** 底下**，選取 [**免費下載**]。**
