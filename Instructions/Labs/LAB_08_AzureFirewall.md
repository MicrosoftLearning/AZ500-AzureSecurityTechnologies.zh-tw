...
10. 在 [新增路由]**** 刀鋒視窗中，指定下列設定：  
   ...
   |下一個躍點位址|您在上一項工作中找出的防火牆私人 IP 位址|
    ...
    >**注意 ** ：Azure 防火牆實際上是受控服務，但虛擬裝置在此情況下運作。
...
5. 在 [新增應用程式規則集合]**** 刀鋒視窗中，使用下列設定在 [目標 FQDN]**** 區段建立新項目 (其他設定請保留預設值)：
   ...
   |目標 FQDNS|**www.bing.com**|

6. 按一下 [新增]****，新增以目標 FQDN 為基礎的應用程式規則。

    >**注意 ** ：Azure 防火牆包含預設允許之基礎結構 FQDN 的內建規則集合。 這些 FQDN 為平台所特有，無法用於其他用途。 
...
5. 在網路介面刀鋒視窗的 [設定 ** ] 區段中，按一下 ** [DNS 伺服器]，選取 ** [自訂 ** ] 選項，新增網路規則中參考的兩部 DNS 伺服器 ** ： ** 209.244.0.3 ** 和 ** 209.244.0.4 ** ，然後按一下 [ ** 儲存 ** ] 以儲存 ** 變更。

6. 返回 [Srv-Work]**** 虛擬機器分頁。

    >**注意 ** ：等候更新完成。

    >**注意 ** ：更新網路介面的 DNS 伺服器會自動重新開機該介面所連結的虛擬機器，如果適用的話，相同可用性設定組中的任何其他虛擬機器。
...
4. 按一下 [下載 RDP 檔案]****，然後使用該檔案透過遠端桌面連線至 **Srv-Jump** Azure VM。 出現驗證提示時，請提供下列認證：
   ...
   |密碼|**Pa55w.rd1234**|
    ...
    >**注意 ** ：您將連線到 ** Srv-Work ** 虛擬機器。 這樣才能測試存取 bing.com 網站的能力。  
