...
系統要求您建立概念證明應用程式，該應用程式會運用 Azure SQL Database 對 Always Encrypted 功能的支援。 此案例中使用的所有秘密和金鑰都應儲存於 Key Vault 中。 應用程式須在 Azure Active Directory (Azure AD) 中註冊，以強化其安全性狀態。 若要達成這些目標，概念證明應該包括：
...
4. **記錄 ADO.NET （SQL 驗證） ** 連接字串。 您稍後會用到它。
