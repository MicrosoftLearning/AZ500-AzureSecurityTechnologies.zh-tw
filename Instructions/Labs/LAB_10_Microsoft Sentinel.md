...
- 工作 2：將 Azure 活動 連線 至 Sentinel
- 工作 3：建立使用 Azure 活動數據連接器的規則。 
- 工作 4：建立劇本
- 工作 5：建立自定義警示，並將劇本設定為自動化回應。
- 工作 6：叫用事件並檢閱相關聯的動作。
...
#### 工作 2：將 Microsoft Sentinel 設定為使用 Azure 活動數據連接器。 

在此工作中，您必須將 Microsoft Sentinel 設定為使用 Azure 活動資料連接器。  

1. 在 [Azure 入口網站] **的 [Microsoft Sentinel \| 概**觀] 刀鋒視窗中，**按兩下 [內容管理**] 區段中的 [**內容中樞**]。

2. 在 [ **Microsoft Sentinel \| 內容中樞** ] 刀鋒視窗上，檢閱可用的內容清單。

3. 在搜尋列中輸入 **Azure** ，然後選取代表 **Azure 活動**的專案。 檢閱其最右邊的描述，然後按兩下 [ **安裝**]。

4. 等候 **安裝成功** 通知。 在左側導覽面板中的 **[組態** ] 區段中，按兩下 [ **資料連接器**]。

5. 在 [ **Microsoft Sentinel \| 數據連接器]** 刀鋒視窗上，按兩下 [ **重新** 整理] 並檢閱可用的連接器清單。 選取代表 **Azure 活動** 連接器的專案（如有需要，請使用 \<<隱藏左側功能表欄）、檢閱其描述和狀態，然後按下 [ **開啟連接器] 頁面**。
...
9. **按兩下 [基本]** 索引標籤底部的 **[下一步]** 按鈕兩次，以繼續進行 [**參數**] 索引標籤。在 [**參數]** 索引標籤上，按兩下 [**主要 Log Analytics] 工作區省略號 （...）** 按鈕。 在 [主要 Log Analytics 工作區****] 頁面中確定已選取您的 Azure Pass 訂用帳戶，並使用 [工作區]**** 下拉式清單選取您要用於 Microsoft Sentinel 的 Log Analytics 工作區。 完成後，按一下頁面底部的 [選取****] 按鈕。

10. 按兩下 [**參數 **] 索引卷標底部的 **[下一步]** 按鈕，繼續進行 [**補救]** 索引標籤。在 [**補救]** 索引標籤上，選取 [**建立補救工作**] 複選框。 這項操作會啟用 [要補救的原則] 下拉式清單中的 [將 Azure 活動記錄設定為串流至指定 Log Analytics 工作區]。**** 在 [系統指派的識別位置****] 下拉式清單中，選取您先前為 Log Analytics 工作區選取的區域 (例如美國東部)。

11. 按兩下 [**補救] 索引卷標底部的 **[下一步**]** 按鈕，繼續前往 [**不符合規範] 訊息索引標籤**。 如果您想要輸入不符合規範的郵件（這是選擇性的），然後按兩下 [不符合規範的郵件 **] 索引卷標底部的 **[**檢閱 + 建立**] 按鈕。

12. 按一下 [建立]**** 按鈕。 您應該會看到三個成功狀態消息： **建立原則指派成功、角色指派建立成功，以及補救工作建立成功**。

    >**注意**：您可以檢查通知、鈴鐺圖示來驗證三個成功的工作。

13. 確認 [Azure 活動****] 窗格中顯示出 [收到的資料****] 圖表 (可能需要重新整理瀏覽器頁面)。  

    >**注意**：狀態顯示「已 連線」之前，可能需要超過15分鐘的時間，圖形會顯示已接收的數據。
...
5. 在 [分析規則精靈 - 建立新的排程規則 **] 刀鋒視窗的 **[**設定規則邏輯**] 索引卷標上，接受預設設定，然後按 **[下一步：事件設定（預覽] >**。
...
8. 在 **[分析規則精靈 - 建立新的排程規則 **] 刀鋒視窗的 **[檢閱和建立**] 索引卷標上，按兩下 [**儲存**]。
...
3. 在 [編輯範本****] 刀鋒視窗中按一下 [載入檔案****]，找出 **\\Allfiles\\Labs\\15\\changeincidentseverity.json** 檔案，然後按一下 [開啟****]。
    ...
    >**注意**：您可以在 找到範例劇本https://github.com/Azure/Azure-Sentinel/tree/master/Playbooks[](https://github.com/Azure/Azure-Sentinel/tree/master/Playbooks)。
...
6. 在 [分析規則精靈] 的 **[**設定規則邏輯**] 索引標籤 - [建立新的排程規則**] 刀鋒視窗的 **[規則查詢**] 文本框中，貼上下列規則查詢。 
...
7. **在 [分析規則精靈 - 建立新的排程規則 **] 刀鋒視窗的 **[設定規則邏輯**] 索引卷標上，於 **[查詢排程**] 區段中，將 [執行查詢] 設定**為 [每** **5 分鐘**]。

8. 在 [分析規則精靈 - 建立新的排程規則 **] 刀鋒視窗的 **[**設定規則邏輯**] 索引卷標上，接受其餘設定的預設值，然後按 **[下一步：事件設定] >**。

9. **在 [分析規則精靈 - 建立新的排程規則 **] 刀鋒視窗的 **[事件設定**] 索引標籤上，接受預設設定，然後按 **[下一步：自動回應] >**。 

10. 在 [分析規則精靈 - 建立新的排程規則 **] 刀鋒視窗的 **[**自動化回應**] 索引標籤上，按兩下 [自動化規則 **] 下的 **[**+ 新增**]。

11. 在 [**建立新的自動化規則] 視窗中，輸入 ****[自動化規則**名稱**執行變更嚴重性劇本**];在 **[觸發**程式] 字段中，按下拉功能表，然後選取 **[建立**警示時]。

12. 在 [**建立新的自動化規則]** 視窗中的 [動作 **] 底下**，閱讀附注，然後按兩下 [**管理劇本許可權**]。 在 [ **管理許可權]** 視窗中，選取先前建立 **的資源群組旁的複選框AZ500LAB1314151** ，然後按兩下 [ **套用**]。

13.  在 [ **建立新的自動化規則]** 視窗中， **單擊 [動作**] 底下的第二個下拉功能表，然後選取 **[變更事件-嚴重性]** 邏輯應用程式。 在 [ **建立新的自動化規則]** 視窗中，按兩下 [ **套用**]。

14. 在 [分析規則精靈 - 建立新的排程規則 **] 刀鋒視窗的 **[**自動回應**] 索引卷標上，按 **[下一步：檢閱並建立>**，然後按兩下 [**儲存]**
...
2. 在 [適用於雲端的 Microsoft Defender 概**觀]** \| 刀鋒視窗中，按兩下**左側導覽中 [雲端安全性 **] 底下的 **[工作負載保護**]。

3. 在 **[適用於雲端的 Microsoft Defender \| 工作負載保護**] 刀鋒視窗上，向下捲動並按兩下 [進階保護 **] 底下的 **[**Just-In-Time VM 存取**] 圖格。

4. 在 [**Just-In-Time VM 存取**] 刀鋒視窗上，在參考 **myVM** 虛擬機的數據列右側，按兩下省略號 （...）** 按鈕，按兩下 ****[移除]，然後按兩下 [**是****]。

    >**注意：** 如果 VM 未列在 Just-In-Time VM** 中**，請流覽至 **[Vm]** 刀鋒視窗，然後按兩下 ****[設定]，按兩下 **[Just-In-Time VM] 下的 **[啟用 Just-In-Time VM****] 選項。 重複執行上述步驟以巡覽回 **適用於雲端的 Microsoft Defender**，然後重新整理頁面，VM 便會出現。
...
8. 在 [ **Microsoft Sentinel \| 概觀** ] 刀鋒視窗上，檢閱儀錶板，並確認它顯示與刪除 Just-In-Time VM 存取原則對應的事件。
...
> 結果：您已建立 Microsoft Sentinel 工作區、將它聯機至 Azure 活動記錄、建立劇本和自定義警示，以回應移除 Just-In-Time VM 存取原則，並確認設定有效。
