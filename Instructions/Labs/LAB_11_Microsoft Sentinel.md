...
6. [Azure 活動****] 刀鋒視窗中的 [說明****] 索引標籤應顯示為已選取。記下**必要條件**，並向下捲動至 [設定****]。 記下說明連接器更新的資訊。 您的訂用帳戶從未使用過舊版連線方法，因此您可以略過步驟 1（ **[中斷連線]** 按鈕將會呈現灰色），然後繼續進行步驟 2。

7. 在步驟 2 **透過診斷設定新管線連線您的訂用帳戶**中，檢閱「啟動 [Azure 原則指派精靈]，並遵循步驟執行」說明，然後按一下[啟動 Azure 原則指派精靈\>****]。

8. 在 [將 Azure 活動記錄設定為串流至指定 Log Analytics 工作區]**** ([指派原則] 頁面) 的 [基本]**** 索引標籤上，按一下 [範圍省略符號 (...)****] 按鈕。 在 [ **範圍** ] 頁面中，從下拉式訂用帳戶清單中選擇您的訂用帳戶，然後按兩下頁面底部的 [ **選取** ] 按鈕。
