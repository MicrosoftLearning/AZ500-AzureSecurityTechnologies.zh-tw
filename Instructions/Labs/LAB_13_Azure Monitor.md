...
2. 在Azure 入口網站的 [**儲存體帳戶**] 刀鋒視窗上，按一下 [**+ 建立**] 按鈕以建立新的儲存體帳戶。
...
4. 在 [**建立儲存體帳戶**] 刀鋒視窗的 [**基本] 索引**標籤上，按一下 [**檢閱**]，等候驗證程式完成，然後按一下 [**建立**]。
...
1. 在Azure 入口網站的 [**搜尋資源]、[服務和檔**] 文字方塊中，于 [Azure 入口網站] 頁面頂端輸入 **[監視**]，然後按**Enter**鍵。

2. 在 [ **監視設定]** 刀鋒視窗上，按一下 [ **資料收集規則]。**
...
3. 在 [**建立資料收集規則**] 刀鋒視窗的 [**基本] 索引**標籤上，指定下列設定：
...
4. 按一下標示為 **[下一步：資源 >** 繼續的按鈕。

5. 在 [ **資源] 索引** 標籤上，選取 **[+ 新增資源**]，然後選取 [ **啟用資料收集端點]。**
...
6. 按一下標示為 **[下一步：收集並傳遞 >** ] 按鈕以繼續進行。

7. 按一下 **[+ 新增資料來源**]，然後在 [ **新增資料來源** ] 頁面上，變更 [資料來源 **類型** ] 下拉式功能表以顯示 **效能計數器。** 保留下列預設設定：
...
8. 按一下標示為 **[下一步：目的地] >** 的按鈕以繼續進行。
  
9. 變更 [ **目的地類型** ] 下拉式功能表以顯示 **Azure 監視器記錄。** 在 [ **訂** 用帳戶] 視窗中，確定您的 [ *訂* 用帳戶] 隨即顯示，然後變更 [ **帳戶] 或 [命名空間** ] 下拉式功能表，以反映您先前建立的 Log Analytics 工作區。
